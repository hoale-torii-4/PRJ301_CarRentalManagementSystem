...
public class UpdatePartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                purchasePrice = Double.parseDouble(request.getParameter("txtPurchasePrice").trim());
                retailPrice = Double.parseDouble(request.getParameter("txtRetailPrice").trim());
            } catch (NumberFormatException e) {
            }
            CarParts carParts = new CarParts(partID, partName, purchasePrice, retailPrice);
            CRUDPartCarDAO partCarDAO = new CRUDPartCarDAO();
            boolean isUpdated = partCarDAO.updateCarPart(carParts);
            if(isUpdated){
